...
- `fixture_loader.py` reads five bundled datasets from `data/` (three drought CSVs in ZIPs + two hydro-station CSVs in ZIPs). Semicolon-separated, `#`-prefixed comment lines. Parses the data timestamp from comment headers. Returns a `DataBundle` including `current_stations_df` and `reference_stations_df`.
...
- Discharge: `regional.py` reads `data/station_region_mapping.json` (station_id → region_id) to join `bundle.current_stations_df` with `bundle.reference_stations_df` (filtered to `label == "Abfluss"`). Results populate `RegionReport.hydro_stations`. The mapping file is generated by `scripts/extract_station_mappings.py`.
...
- `DataBundle` — raw DataFrames (current, historic, reference, forecast) + source tag. `forecast_df`, `current_stations_df`, and `reference_stations_df` all default to empty DataFrames.
- `HydroStationReport` — discharge reading for one hydro station: `station_id`, `current_value`, `threshold1`, `min_value`.
...
- `RegionReport` — per-region indicators: CDI (0–5), SPI, soil moisture, VHI, precip sums, sub-index levels (precip_1m_index / soil_moisture_index / hydro_index, each 1–5), warnlevel, quality, `hydro_stations` (list of `HydroStationReport`).
...
1. Full-width warnlevel badge
2. Two equal columns: left = `allgemeine-lage` section text; right = map switcher + CDI legend
3. Full-width remaining sections (`handlungsoptionen`, `datenquellen`)
